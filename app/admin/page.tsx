...
export default function AdminDashboard() {
  ...
  // NEW: State to show a loading spinner while the image is uploading
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Send the file silently to ImgBB using your custom API Key
      ...
      if (data.success) {
        // Automatically save the secure URL to our blog form!
  ...
  const handleBlogSubmit = async (e: React.FormEvent) => {
    ...
    // Safety check in case they click publish before the image finishes uploading
    if (isUploading) {
  ...
  const handleWhatsAppMessage = (name: string, phone: string, date: string) => {
    let cleanPhone = phone.replace(/\D/g, '');
    if (cleanPhone.length === 10) {
      cleanPhone = '91' + cleanPhone;
    }
    const message = `Hello ${name}, this is the reception at Vraj Homeopathy. 🏥\n\nWe received your appointment request for ${date}. Could you please let us know what time works best for you to visit the clinic?`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="h-10 w-10 bg-slate-100 rounded-full border-2 border-slate-200 flex items-center justify-center text-slate-600 font-black shadow-sm">
                SV
              </div>
            </div>
          </div>
                        ...
                        <tr key={appt._id} className="hover:bg-slate-50/80 transition-colors group">
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              {/* WHATSAPP BUTTON */}
                              <button 
                                onClick={() => handleWhatsAppMessage(appt.name, appt.phone, appt.date)}
                    ...
                    <div key={appt._id} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className="flex items-center gap-1">
                          <button onClick={() => handleWhatsAppMessage(appt.name, appt.phone, appt.date)} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-md">
              ...
              <form onSubmit={handleBlogSubmit} className="p-6 space-y-5 bg-slate-50/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    {/* Visual checkmark when the image successfully attaches */}
